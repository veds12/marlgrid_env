...
class AddStateSpaceActMaskWrapper(gym.core.Wrapper):
    # Some algorithms require a state space and action masks
    def __init__(self, env):
        super().__init__(env)
        obs_shape = self.observation_space[0].shape
...
class ParallelEnv(gym.Env):
    """
    Adapted from https://github.com/mila-iqia/babyai/blob/master/babyai/rl/utils/penv.py
...
def WorkerStateSpaceActMask(conn, env):
    while True:
        cmd, data = conn.recv()
        # print(f'Received command: {cmd} and action {data}')
        if cmd == "step":
...
class ParallelEnvStateSpaceActMask(gym.Env):
    """
    A concurrent execution of environments in multiple processes.
    Works with an env with AddStateSpaceActMaskWrapper
    Designed to work with saf codebase interface
    """
    ...
    def __init__(self, envs):
        ...
        for env in self.envs[1:]:
            local, remote = mp.Pipe()
            self.locals.append(local)
            p = mp.Process(target=WorkerStateSpaceActMask, args=(remote, env))
            p.daemon = True
            p.start()
            remote.close()
            self.processes.append(p)
